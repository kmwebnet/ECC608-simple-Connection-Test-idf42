...
ATCA_STATUS hal_i2c_send(ATCAIface iface, uint8_t address, uint8_t *txdata, int txlength)
{
    ...
    if (txlength > 0){
        (void)i2c_master_write_byte(cmd, (address << 1) | I2C_MASTER_WRITE, ACK_CHECK_EN);
        (void)i2c_master_write(cmd, txdata, txlength, ACK_CHECK_EN);
    }
    else
    {
        (void)i2c_master_write_byte(cmd, I2C_MASTER_WRITE, ACK_CHECK_EN);
    }
...
ATCA_STATUS hal_i2c_receive(ATCAIface iface, uint8_t address, uint8_t *rxdata, uint16_t *rxlength)
{
    ...
    if ((NULL == cfg) || (NULL == rxlength) || (NULL == rxdata))
    {
        ...
        (void)i2c_master_start(cmd);
        (void)i2c_master_write_byte(cmd, (address << 1) | I2C_MASTER_READ, ACK_CHECK_EN);
        if (*rxlength > 1)
        {
...
ATCA_STATUS hal_i2c_idle(ATCAIface iface)
{
    ...
    (void)i2c_master_start(cmd);
    (void)i2c_master_write_byte(cmd, cfg->atcai2c.address | I2C_MASTER_WRITE, ACK_CHECK_EN);
